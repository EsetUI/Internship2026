...
def build_screen(width: int,height: int,caption: str,*bodies: str) -> str:
	...
	# vertically center answers
	start_row = (height // 2) - (len(bodies) // 2)

	for index, body in enumerate(bodies):
		place_centered_text(lines,start_row + index,body)

	return "\n".join(
		colorize_line(line, row, height)
...
def render_loop() -> None:
	...
	try:
		while True:
			...
			screen = build_screen(width,height,question_data["question"],*question_data["answers"])
			prefix = CURSOR_HOME
